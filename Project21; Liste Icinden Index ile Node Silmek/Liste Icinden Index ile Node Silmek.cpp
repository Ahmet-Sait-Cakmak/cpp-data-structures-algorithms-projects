...
public:
	...
	void deleteNode(int _index) {
		...
		if (_index < 0 || _index >= number_of_nodes) {
		...
		else if (_index == number_of_nodes - 1) {
			...
			if (_index <= number_of_nodes / 2) {
				...
				adressHolder->adress_of_previousNode->adress_of_nextNode = adressHolder->adress_of_nextNode;
				adressHolder->adress_of_nextNode->adress_of_previousNode = adressHolder->adress_of_previousNode;

				delete adressHolder;
			}
			else {

				Node* adressHolder = tail;
...
int main() {
	...
	adress_of_doublyLinkedList1->deleteNode(2);

	cout << "\nAfter deleting the node at index 2 in the linked list: " << endl << endl;
