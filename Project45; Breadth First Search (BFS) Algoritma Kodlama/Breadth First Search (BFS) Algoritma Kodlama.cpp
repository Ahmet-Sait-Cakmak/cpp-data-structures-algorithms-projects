...
public:
	...
	void BreadthFirstSearch() {
		...
		while (!queue1.empty()) {
			...
			Node* adressHolder = queue1.front();
			...
			if (adressHolder->adress_of_leftChild != NULL) {
				...
				queue1.push(adressHolder->adress_of_leftChild);
			}

			if (adressHolder->adress_of_rightChild != NULL) {

				queue1.push(adressHolder->adress_of_rightChild);
			}
		}
	}
};
...
int main() {
	...
	cout << "Breadth First Search Algoritmasi ile yazilmis binary search tree : ";

	binarySearchTree1->BreadthFirstSearch();
